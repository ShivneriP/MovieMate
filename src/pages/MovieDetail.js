 ...
 export const MovieDetail = () => {
   ...
   useEffect(()=>{
    async function fetchMovie(){
      const response= await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=605df98f85722795ddd33c0c5553c7ca`);
      ...
      <section className='flex justify-around flex-wrap py-5'>
        <div className='max-w-sm'>
         ...
         <div className='max-w-2xl text-gray-700 text-lg dark:text-white'>
          ...
          {movie.genres?(<p className='m-7 flex flex-wrap gap-2'>
              {movie.genres.map((genre)=>(<span className='mr-2 border border-gray-200 rounded dark:border-gray-600 p-2 ' key={genre.id}>{genre.name}</span>
            ))}
            </p>):""}
